...
def make_3d_sweep_around_axis(contour, num_steps=40, color="r"):
    ...
    # smooth the 2D contour before sweep
    contour[:, 0] = gaussian_filter1d(contour[:, 0], sigma=2)
    contour[:, 1] = gaussian_filter1d(contour[:, 1], sigma=2)
...
# Filter predicted masks by GT area
...
# Visualize 3D
vp = Plotter(title="Smoothed 3D GT vs Filtered Predicted", axes=1, bg="white")
gt_count = 0
pred_count = 0
